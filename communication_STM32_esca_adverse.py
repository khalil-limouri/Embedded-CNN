...
class ESCA_set:
    ...
    def pick_rand_value_from_xtest(self):
        ...
        x_sample = self.X_test[rand_sample]
        image=tf.convert_to_tensor(x_sample)
        image = image[None, ...]
        loss_object = tf.keras.losses.CategoricalCrossentropy()
          
        with tf.GradientTape() as tape:
            tape.watch(image)
            model = tf.keras.models.load_model('model_b32\model_small_b32.h5')
            prediction = model(image)
            retriever_index = 60
            label = tf.one_hot(retriever_index, prediction.shape[-1])
        ...
        adv_x = image + 0.5*perturbations
        adv_x = tf.clip_by_value(adv_x, -1, 1)
        adv_sample = adv_x.numpy()
        
        self.x_sample = adv_sample
        self.y_sample = self.Y_test[rand_sample]
        tmp = self.y_sample.argmax(axis=0)
        print("Chosen input's corresponding label is "+labels[tmp]+" according to y_test")
...
def send_NN_inputs_to_STM32(ESCA_set, ser):
    ...
    tmp = ESCA_set.x_sample.reshape((80,45,3))
...
if __name__ == '__main__':
    ...
    with serial.Serial("COM3", 115200, timeout=1) as ser: # COM5 for H743 (nucleo) and COM6 for F411 (Nucleo)
        ...
        used_model = "model_b32\model_small_b32.h5"
    
        # X_test and Y_test dataset available for inference
        path_xtest = "esca_xtest_small.npy"
        path_ytest = "esca_ytest_small.npy" 
        i = 0
        nb_error = 0
        errored_elem = []
